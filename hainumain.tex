% !TeX program = lualatex
% !Mode:: "TeX:UTF-8"

\documentclass[12pt,openany,oneside]{ctexbook}                                  %采用最新的中文宏包                  % 本科生毕业论文通常采用单页排版
\input{setup/package}                                % 定义本文所使用宏包
\graphicspath{{figures/}}                            % 定义所有的 .eps 文件在 figures 子目录下
\newcommand{\defaultfont}{\songti}                   % 开始中文字体使用

\begin{document}                                     % 开始全文
\input{setup/format}                                 % 完成对论文各个部分格式的设置
\frontmatter                                         % 以下是论文导言部分，包括论文的封面，中英文摘要和中文目录
\fancypagestyle{plain}{
  \fancyhf{}
  \fancyhead[C]{\song\wuhao \cheadingtitle}          % 页眉使用论文标题
  \fancyfoot[C]{\song\xiaowu ~\thepage~}         % 页脚显示页码
  \renewcommand{\headrulewidth}{0.5pt}
  \renewcommand{\footrulewidth}{0pt}
}
\pagenumbering{Roman}
\input{preface/cover}                                % 封面

%%%%%%%%%%   目录   %%%%%%%%%%
\defaultfont
\clearpage
\titleformat{\chapter}{\centering\sanhao\song}{\chaptername}{2em}{} % 设置目录两字的格式
\pdfbookmark[0]{目~~录}{mulu}
\tableofcontents                                     % 中文目录
\thispagestyle{plain}

\mainmatter\defaultfont\sloppy\raggedbottom
\makeatletter
\fancypagestyle{plain}{                              % 设置开章页眉页脚风格
    \fancyhf{}
    \fancyhead[C]{\song\wuhao \@cheading}            % 首页页眉格式
    \fancyfoot[C]{\song\xiaowu ~\thepage~}           % 首页页脚格式
    \renewcommand{\headrulewidth}{0.5pt}
    \renewcommand{\footrulewidth}{0pt}
}
\makeatother
\setcounter{page}{1}                                 % 单独从 1 开始编页码
\titleformat{\chapter}{\sanhao\song\bfseries}{\chaptername}{1em}{}
                                                     % 恢复chapter 标题格式要求
\include{body/intros}
\include{body/figures}
\include{body/tables}
\include{body/equations}
\include{body/others}
\titleformat{\chapter}{\centering\sanhao\song\bfseries}{\chaptername}{1em}{}
\include{appendix/acknowledgements}            % 致谢
%%%%%%%%%%  参考文献  %%%%%%%%%%
\defaultfont
\bibliographystyle{HNUThesis}
\phantomsection
\addcontentsline{toc}{chapter}{参考文献}          % 参考文献加入到中文目录
\nocite{*}                                        % 若将此命令屏蔽掉，则未引用的文献不会出现在文后的参考文献中。
\bibliography{references/reference}

%附件
\include{appendix/attachment}
            % 外文资料
% \include{appendix/paperInEnglish}              % 外文资料
% \include{appendix/paperInChinese}              % 中文译文

\clearpage
                                 % 结束中文字体使用
\end{document}                                 % 结束全文
